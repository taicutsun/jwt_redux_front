...
useEffect(() => {
  if(click>=1){
    ...
    .then((res) =>{
      ...
      if(res.data.status===true){setStatus(true);
      dispatch(setUserName(prop.user));dispatch(setLogK(pending));
      tokens.accessToken=res.data.accessToken; tokens.refreshToken=res.data.refreshToken;
      setClick(0);cl=0
    }
    else if(res.data.status===false){ dispatch(setLogF(failed));setClick(0); }
    })
    .catch(err=>{
        if (err.response) { console.log('why');}
         else if (err.request) { console.log('req')} 
         else { console.log('me')} 
    });
  }
},);
if (status){ return ( <Navigate to="/posts" /> ); }
else{
return(
<div>
    <button className='loginButt' onClick={ ()=>{setClick(click+1);cl++ } }>Войти</button>
    <LogMass />
</div>
);}
}
