...
export const userSlice = createSlice({
  ...
  reducers: {
    ...
    createUser:(state:UserState, action: PayloadAction<CheckUserPass>):void => {
        state.username=action.payload.username;
        state.password=action.payload.password;
        if(state.password===action.payload.secPass && state.password!==''){ axSendU(state.username,state.password); };
